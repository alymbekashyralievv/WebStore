...
const ProductDetails = ({ productId = 20}) => {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                let { data: product, error } = await supabase.from("sup_products")
                    .select("*")
                    .eq("id", productId);
                console.log(product);
    ...
    if (!product) return <p className="text-center text-red-500 text-lg">Товар не найден</p>;

    const { inch, system, camera, charge, name, description, color, memory, rel_date,type, resolution, ppi , display, price, color_depth    } = product[0]
            ...
            <div className="flex  gap-[20px]">
                <div className="border border-[red]">
                    ...
                    <div className="flex justify-between mb-[32px] mr-[300px]">
                        {product[0].images?.map((url, index) => (
                            <img className="max-w-[250px] min-w-[100px] h-[222px] " key={index} src={url} alt="photo" />
                        ))}
                    </div>
                    <div className="border-2 p-[20px] border-[#F2F5F9] rounded-[12px]">
                        ...
                        <div className="text-gray-500 flex gap-[20px] my-[20px] ">
                            <p className="bg-[#F6F7FA] rounded-[8px] py-[8px] px-[12px]">Камера <span className="text-black">{camera}</span> </p>
                            <p className="bg-[#F6F7FA] rounded-[8px] py-[8px] px-[12px]">Система <span className="text-black">{system}</span> </p>
                            <p className="bg-[#F6F7FA] rounded-[8px] py-[8px] px-[12px]" >Диагональ <span className="text-black">{inch}</span> </p>
                            <p className="bg-[#F6F7FA] rounded-[8px] py-[8px] px-[12px]">Зарядка <span className="text-black">{charge}</span> </p>
                        </div>
                        <div>
                            <div>
                                <button
                ...
                <div className="border border-[#f43232]">
                    ...
                    <div className="flex gap-[30px]">
                        ...
                        <div className="flex text-red-500"><span>4.0</span> <span className="flex"><GiRoundStar /><GiRoundStar /><GiRoundStar /><GiRoundStar /><SlStar /></span> 447 Отзывов</div>
                    </div>
                    <div>
                        <div>
                            <h3>Дизайн</h3>
                            ...
                            <div className="flex gap-[3px]">
                                <div className="bg-red-500 w-20 h-1.5 rounded-[10px] "></div>
                                <div className="bg-red-500  w-20 h-1.5 rounded-[10px] "></div>
                                <div className="bg-red-500  w-20 h-1.5.5 rounded-[10px]"></div>
                                <div className="bg-red-500  w-20 h-1.5 rounded-[10px] "></div>
                                <div className="bg-gray-300  w-20 h-1.5 rounded-[10px]"></div>
                            </div>
                            <h3>Камера</h3>
                    ...
                    <div>
                        <div className="flex gap-2 mt-2">
                            ...
                            {[
                                ...
                                <button key={index} className={`w-10 p-[4px]  h-10 rounded-[8px] border-2 
                                    ${color === el.name_color ? 'border-[#FF4D4D]' : 'border-gray-300'}  bg-[${el.hex_color}]`}></button>
                            ))}
                        </div>
                        ...
                        <div className="flex gap-2 mt-2">
                            ...
                            {memory.map((m, index) => (
                                <button className="px-4 py-2 border-2 border-gray-300 rounded text-gray-500" key={index}>{m} ГБ</button>
                            ))}
                            {/* <button className="px-4 py-2 border-2 border-red-500 rounded text-black">256 ГБ</button> */}
                        </div>
                    </div>
